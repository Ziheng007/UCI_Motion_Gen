    ...
    def __init__(
        ...
        *,
        ...
        for _ in range(depth):
            mlp = Mlp(in_features=dim, hidden_features=dim*4, drop=attn_dropout)
            # frames moamask MMM
            time_attn = nn.TransformerEncoderLayer(d_model=dim,
                                                    nhead=heads,
                                                    dim_feedforward=dim*4,
                                                    dropout=attn_dropout,
                                                    activation='gelu')
            # body attention (same frame)
            spatial_attn = nn.TransformerEncoderLayer(d_model=dim,
                                        nhead=heads,
                                        dim_feedforward=dim*4,
                                        dropout=attn_dropout,
                                        activation='gelu')
            ...
            if shift_tokens:
                time_attn, spatial_attn, mlp = map(lambda t: PreTokenShift(num_frames, t), (time_attn, spatial_attn, mlp))

            time_attn, spatial_attn, mlp = map(lambda t: PreNorm(dim, t), (time_attn, spatial_attn, mlp))

            self.layers.append(nn.ModuleList([time_attn, spatial_attn, mlp]))

    def forward(self, x, mask = None):
        b, f, _, h, w, *_, device, ph, pw = *x.shape, x.device, self.patch_height, self.patch_width
        ...
        x = rearrange(x, 'b f c (hp ph) (wp pw) -> b (f hp wp) (ph pw c)', ph=ph, pw=pw)
        ...
        frame_mask = mask.repeat(n,1)
        # if exists(mask):
        #     # mask_with_cls = F.pad(mask, (1, 0), value = True)
        #     frame_mask = repeat(mask, 'b f -> (b h n) () f', n = n, h = self.heads)
        # time and space attention
        # [bs,4,seq_len,d] ->  [bs,seq_len,4,codebook]
        # multi vq 
        for (time_attn, spatial_attn, ff) in self.layers:
            x = rearrange(x, 'b (f n) d -> f (b n) d',n = n, f = f)
            x = time_attn(x, src_key_padding_mask=frame_mask) + x
            x = rearrange(x, 'f (b n) d -> n (b f) d',n = n, f = f)
            x = spatial_attn(x) + x
            x = rearrange(x, 'n (b f) d -> b (f n) d', n=n, f=f)
            x = ff(x) + x
        return x