...
if __name__ == '__main__':
    ...
    est_length = False
    if opt.text_prompt != "":
        # prompt_list.append([opt.text_prompt + opt.text_l_arm])
        # prompt_list.append([opt.text_prompt + opt.text_r_arm])
        # prompt_list.append([opt.text_prompt + opt.text_l_leg])
        # prompt_list.append([opt.text_prompt + opt.text_r_leg])
        temp = []
        original_list.append(opt.text_prompt)
    ...
    if est_length:
        ...
        text_embedding = t2m_transformer.encode_text(opt.text_prompt)
    ...
    for r in range(opt.repeat_times):
        print("-->Repeat %d"%r)
        with torch.no_grad():
            mids = t2m_transformer.generate(captions[0], token_lens,
                                            timesteps=opt.time_steps,
                                            cond_scale=opt.cond_scale,
                                            temperature=opt.temperature,
                                            topk_filter_thres=opt.topkr,
                                            gsample=opt.gumbel_sample)
            mids = res_model.generate(mids, captions[0], token_lens, temperature=1, cond_scale=5)
        ...
        for k, (caption, joint_data)  in enumerate(zip(original_list, data)):
            print("---->Sample %d: %s %d"%(k, caption, m_length[k]))
            animation_path = pjoin(animation_dir, str(k))
            joint_path = pjoin(joints_dir, str(k))
            raw_path = pjoin(joint_path, "raw")
            os.makedirs(animation_path, exist_ok=True)
            os.makedirs(joint_path, exist_ok=True)
            os.makedirs(raw_path, exist_ok=True)
            
            joint_data = joint_data[:m_length[k]]
            np.save(pjoin(raw_path, "raw_sample%d_repeat%d_len%d.npy"%(k, r, m_length[k])), torch.from_numpy(joint_data).float())
