...
from models.mask_transformer.transformer import MaskTransformer,TimesTransformer
# from models.mask_transformer.transformer_trainer import MaskTransformerTrainer_recap
...
# from data.t2m_dataset import FineText2MotionDataset
...
def load_vq_model(vq_opt,mean,std):
    ...
    vq_model = RVQVAE_Decode(vq_opt,
                ...
                mean,
                std)
    ckpt = torch.load(pjoin(vq_opt.checkpoints_dir, vq_opt.dataset_name, vq_opt.name, 'model', 'latest.tar'),map_location=opt.device)
...
def load_and_freeze_clip(opt, clip_version):
...
def encode_text(opt):
    ...
    id_list = os.listdir(opt.motion_dir)
    min_motion_len = 40 if opt.dataset_name =='t2m' else 24
    for name in tqdm(id_list):
        # name = name.split('.')[0]
        name ='005313'
        # if os.path.exists(pjoin(opt.recaption_text_dir, 'emb', name + '.npy')):
        #     continue
        ...
        with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
            for line in f.readlines():
                ...
                caption = [line_split[0] for _ in range(4)]
                for idx in range(4):
                    key = BODY_PARTS[idx]
                    caption[idx] = [caption[idx] + ' ' + recaption_text_dict[key]]
                    print(recaption_text_dict[key])
                print('caption:', caption)
                cond_vector = []
                for cond in caption:
                    text = clip.tokenize(cond, truncate=True).to(device)
                    cond_vector_ = clip_model.encode_text(text).float()
                    cond_vector.append(cond_vector_)
                cond_vector = torch.cat(cond_vector, dim=0).detach().cpu().numpy()
                # np.save(pjoin(opt.recaption_text_dir, 'emb', name + '.npy'), cond_vector)
...
if __name__ == '__main__':
    ...
    # if opt.base_name != "":
    #     opt.base_root = pjoin(opt.checkpoints_dir, opt.dataset_name, opt.base_name)
    #     opt.base_model_dir = pjoin(opt.base_root, 'model')
    ...
    opt.recaption_text_dir = pjoin(opt.data_root, 're-captions_refined')
    ...
    opt.recaption_text_dir = pjoin(opt.data_root, 're-captions_refined')
    if not os.path.exists(pjoin(opt.recaption_text_dir,'emb')):
        # os.makedirs(pjoin(opt.recaption_text_dir,'emb'))
        print('Loading CLIP...')
        import pdb;pdb.set_trace()
        clip_model = load_and_freeze_clip(opt, clip_version)
        encode_text(opt)

    opt.num_tokens = vq_opt.nb_code

    t2m_transformer = TimesTransformer(code_dim=vq_opt.code_dim,
                                      cond_mode='text',
    ...
    train_dataset = FineText2MotionDataset(opt, mean, std, train_split_file)
    val_dataset = FineText2MotionDataset(opt, mean, std, val_split_file)
    ...
    eval_val_loader, _ = get_dataset_motion_loader(dataset_opt_path, 32, 'val', device=opt.device, is_recap=True)

    wrapper_opt = get_opt(dataset_opt_path, torch.device('cuda'))
    eval_wrapper = EvaluatorModelWrapper(wrapper_opt)

    trainer = MaskTransformerTrainer_recap(opt, t2m_transformer, vq_model)
